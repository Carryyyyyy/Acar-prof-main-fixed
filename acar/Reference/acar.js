...
class acar {
    static komutYükle() {
        fs.readdir("./acar/Moderation", (err, files) => {
            ...
            console.log('\x1b[36m%s\x1b[0m', `\n[ CARRY Moderasyon (${files.length}) adet Komut Yüklendi! 
            Önemli ! = Bot Acar'ın Alt Yapısı Olup Çoğu Hatası ! Carryyyyyy#0001 -(740964663239508020) Tarafından Düzeltilmiştir İyi Kullanımlar.]`);
            files.forEach(file => {
    ...
    static On() {
      global.client.on('ready', () => {
            client.user.setPresence({ activity: { name: sistem.botdurumu }, status: "idle" });
            if (client.channels.cache.has(sistem.botseskanal)) client.channels.cache.get(sistem.botseskanal).join().catch();
        });
       global.client.login(global.sistem.token).catch(err => console.error("[ CARRY Professional Main ] Discord API Botun tokenini doğrulayamadı."));
    }

    static Moderation() {
      console.log("\x1b[33m%s\x1b[0m", "[ CARRY Moderation ] Çalıştırıldı!")
    ...
    static sistemÇalıştır(sistemismi) {
      if(sistemismi === "coin") {
        if(coinsistem.sistem) {
          console.log("\x1b[33m%s\x1b[0m", "[ CARRY Coin System ] Çalıştırıldı!")
        ...
        if(terfisistem.sistem) {
          console.log("\x1b[33m%s\x1b[0m", "[ CARRY Auto Staff System ] Çalıştırıldı!")
