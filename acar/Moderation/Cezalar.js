...
module.exports = {
  ...
  let aylar = aylartoplam;
        ...
        message.channel.wsend(`:no_entry_sign: <@${uye.id}> kişisinin ceza bilgileri aşağıda belirtilmiştir. Tekli bir cezaya bakmak için \`.ceza bilgi @Carry/ID\` komutunu uygulayınız.\n\`\`\`${veriler}\`\`\``).catch(acar => {
            let dosyahazırla; 
            dosyahazırla = new MessageAttachment(Buffer.from(veriler), `${uye.id}-cezalar.txt`);
            message.channel.wsend(`:no_entry_sign: <@${uye.id}> kişisinin cezaları **Discord API** sınırını geçtiği için metin belgesi hazırlayıp gönderdim, oradan cezaları kontrol edebilirsin.\nTekli bir cezaya bakmak için \`.ceza bilgi @Carry/ID\` komutunu uygulayınız.`, dosyahazırla); 
        });
    } else {
