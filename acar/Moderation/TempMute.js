...
module.exports = {
    Isim: "chatmute",
    Komut: ["mute","sustur"],
    Kullanim: "chatmute <@Carry/ID> <1s-1m-1h-1y> <Sebep>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!uye) return message.channel.wsend(cevaplar.üye + ` \`${sistem.prefix}${module.exports.Isim} <@Carry/ID> <1s/1m/1d/1y> <Sebep>\``);
    ...
    if(!sure || !ms(sure)) return message.channel.wsend(cevaplar.süre + ` \`${sistem.prefix}${module.exports.Isim} <@Carry/ID> <1s/1m/1d/1y> <Sebep>\``);
    ...
    if (!mutegetir.some(m => m.id == uye.id)) { 
      ...
      uye.roles.add(roller.muteRolü).catch(err => console.log(`[~ CARRY ~] Mute Rolü eklerken hata oluştu!`));
