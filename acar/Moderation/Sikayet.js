...
module.exports = {
    Isim: "şikayet",
    Komut: ["sikayet"],
    Kullanim: "şikayet <@Carry/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>",
    Aciklama: "şikayet <@Carry/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>",
    Kategori: "Üye",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!uye) return message.channel.wsend(cevaplar.üye + ` \`${sistem.prefix}${module.exports.Isim} <@Carry/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\``).then(x => x.delete({timeout: 5000})); 
    ...
    let tür = args[1];
    if(tür !== "chat" && tür !== "sesli" && tür !== "taciz" && tür !== "dm") return message.channel.wsend(cevaplar.argümandoldur + ` \`${sistem.prefix}${module.exports.Isim} <@Carry/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\``).then(x => x.delete({timeout: 5000})); 
    let şikayetbilgi = args.slice(2).join(' ');
    if(!şikayetbilgi) return message.channel.wsend(cevaplar.argümandoldur + ` \`${sistem.prefix}${module.exports.Isim} <@Carry/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\``).then(x => x.delete({timeout: 5000})); 
